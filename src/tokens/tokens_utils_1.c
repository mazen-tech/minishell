...
//processing the tokens accourding it type
...
{
    if (s[*len] == '|' && s[*len - 1] != '|')//checking if the current char at index *len ia a pipe '|'
    {//and previous char not a pipe
        *len = *len - 1;
        att->pars_data.nb_tokenst++;
        return (1);
    }//checking if the current char and previous char are both pipes '||'
